...
let AJUSTE_CAPITAL = Number(localStorage.getItem('ajusteCapital')) || 0;
...
async function cargarResumenCapital() {
    try {
        ...
        // Fórmula: Capital en mano = (Lo que puse al inicio + Ajustes - Lo que salió en préstamos + Lo que ya cobré)
        const capitalTotalActual = CAPITAL_TOTAL_DINAMICO + AJUSTE_CAPITAL - totalMontoPrestadoHistorico + totalRecaudadoHistorico;
        ...
        const gananciaFutura = proyeccion - (CAPITAL_TOTAL_DINAMICO + AJUSTE_CAPITAL);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- LÓGICA PARA AGREGAR/RETIRAR CAPITAL EN EL MODAL DE PROYECCIÓN ---
    const inputAjusteCapital = document.getElementById('input-ajuste-capital');
    if (inputAjusteCapital) {
        inputAjusteCapital.addEventListener('input', (e) => {
            let valor = e.target.value.replace(/\D/g, "");
            if (valor) {
                valor = parseInt(valor).toLocaleString('es-AR');
                e.target.value = valor;
    ...
    const btnAgregarCapital = document.getElementById('btn-agregar-capital');
    const btnRetirarCapital = document.getElementById('btn-retirar-capital');

    const modalConfirmarAjuste = document.getElementById('modal-confirmar-ajuste');
    ...
    const textoConfirmarAjuste = document.getElementById('texto-confirmar-ajuste');

    let ajustePendiente = { esAgregar: true, monto: 0 };

    function prepararAjuste(esAgregar) {
        if(!inputAjusteCapital) return;
        const valorConPuntos = inputAjusteCapital.value;
        if (!valorConPuntos) return;
        const montoAjuste = Number(valorConPuntos.replace(/\./g, ''));
        if (montoAjuste <= 0) return;

        if (esAgregar) {
            if (textoConfirmarAjuste) textoConfirmarAjuste.innerHTML = `Estás a punto de <strong>agregar ${formatearMoneda(montoAjuste)}</strong> a tu capital total.`;
        } else {
            // Validar que no retire más del capital disponible
            const capitalDisponibleUI = document.getElementById('total-disponible').innerText;
            const capitalDisponible = Number(capitalDisponibleUI.replace(/\D/g, ''));
            if (montoAjuste > capitalDisponible) {
                alert(`No puedes retirar más del capital que tienes disponible (${formatearMoneda(capitalDisponible)}).`);
                return;
            }
            if (textoConfirmarAjuste) textoConfirmarAjuste.innerHTML = `Estás a punto de <strong>retirar ${formatearMoneda(montoAjuste)}</strong> de tu capital total.`;
        }

        ajustePendiente = { esAgregar, monto: montoAjuste };
        if (modalProyeccion) modalProyeccion.style.display = 'none';
        if (modalConfirmarAjuste) modalConfirmarAjuste.style.display = 'flex';
    }

    if (btnCancelarAjuste) {
        btnCancelarAjuste.onclick = () => {
            if (modalConfirmarAjuste) modalConfirmarAjuste.style.display = 'none';
            if (modalProyeccion) modalProyeccion.style.display = 'flex';
        };
    }

    if (btnConfirmarAjuste) {
        btnConfirmarAjuste.onclick = async () => {
            btnConfirmarAjuste.disabled = true;
            btnConfirmarAjuste.innerText = "Guardando...";

            if (ajustePendiente.esAgregar) {
                AJUSTE_CAPITAL += ajustePendiente.monto;
            } else {
                AJUSTE_CAPITAL -= ajustePendiente.monto;
            }

            localStorage.setItem('ajusteCapital', AJUSTE_CAPITAL);

            btnConfirmarAjuste.disabled = false;
            btnConfirmarAjuste.innerText = "Confirmar";

            if (inputAjusteCapital) inputAjusteCapital.value = ""; 
            if (modalConfirmarAjuste) modalConfirmarAjuste.style.display = 'none';
            cargarResumenCapital(); 
        };
    }

    if (btnAgregarCapital) btnAgregarCapital.onclick = () => prepararAjuste(true);
    if (btnRetirarCapital) btnRetirarCapital.onclick = () => prepararAjuste(false);
